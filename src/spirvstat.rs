...
mod tests {
    ...
    /// Four entry points, each reachable, each with a body — a report
    /// of zero would mean the walker missed the function section.
    #[test]
    fn the_shipping_module_reads_back_with_all_of_its_entry_points() {
        ...
        assert_eq!(names, vec!["vs_main", "fs_main", "fs_blur", "fs_image"]);
        for cost in m.costs() {
    ...
    /// samples a texture, no fragment of this branch takes a screen
    /// derivative (the shape lane, which does, is not on this branch),
    /// and the vertex stage touches no texture at all.
    #[test]
    fn the_counts_answer_to_what_the_shader_source_says() {
        ...
        for c in m.costs() {
            assert_eq!(
                ...
                0,
                "{} takes a derivative, and no shader on this branch should",
                c.name
            );
        }
    }
    ...
    fn the_wgsl_is_lifted_out_of_a_rust_source_that_carries_it() {
        ...
        assert_eq!(from_wgsl(lifted).unwrap().entries.len(), 4);
    }
    ...
    fn the_report_names_the_entry_points_and_both_numbers() {
        ...
        for name in ["vs_main", "fs_main", "fs_blur", "fs_image"] {
